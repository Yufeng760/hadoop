...
(function ($) {
    $.extend({
        ...
        table: {
            ...
            selectColumnsByTableID: function (table_id,column) {
                var rows = $.map($("#" + table_id).bootstrapTable('getSelections'), function (row) {
                    return row[column];
                });
                if ($.common.isNotEmpty(table.options.rememberSelected) && table.options.rememberSelected) {
                    var selectedRows = table.rememberSelecteds[table_id];
                    if ($.common.isNotEmpty(selectedRows)) {
                        rows = $.map(table.rememberSelecteds[table_id], function (row) {
                            return row[column];
                        });
                    }
                }
        ...
        operate: {
            ...
            successTabCallback: function (result) {
                if (result.code == web_status.SUCCESS) {
                    ...
                    var $contentWindow = $('.dainfo_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow;
                    $.modal.close();
